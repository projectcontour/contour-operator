...
const (
	...
	// latestRelease is the "from" version used to test operator upgrades.
...
func TestDefaultContour(t *testing.T) {
	...
	if err := newNs(ctx, kclient, cfg.Namespace); err != nil {
		t.Fatalf("failed to create namespace %s: %v", cfg.Namespace, err)
	}
	...
	if isKind {
		svcName := "envoy"
		if err := updateLbSvcIPAndNodePorts(ctx, kclient, timeout, cfg.Namespace, svcName); err != nil {
			t.Fatalf("failed to update service %s/%s: %v", cfg.Namespace, svcName, err)
		}
		t.Logf("updated service %s/%s loadbalancer IP and nodeports", cfg.Namespace, svcName)
	}
	...
	if err := newDeployment(ctx, kclient, appName, cfg.Namespace, testAppImage, testAppReplicas); err != nil {
		t.Fatalf("failed to create deployment %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("created deployment %s/%s", cfg.Namespace, appName)

	if err := waitForDeploymentStatusConditions(ctx, kclient, timeout, appName, cfg.Namespace, expectedDeploymentConditions...); err != nil {
		t.Fatalf("failed to observe expected status conditions for deployment %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("observed expected status conditions for deployment %s/%s", cfg.Namespace, appName)

	if err := newClusterIPService(ctx, kclient, appName, cfg.Namespace, 80, 8080); err != nil {
		t.Fatalf("failed to create service %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("created service %s/%s", cfg.Namespace, appName)

	if err := newIngress(ctx, kclient, appName, cfg.Namespace, appName, 80); err != nil {
		t.Fatalf("failed to create ingress %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("created ingress %s/%s", cfg.Namespace, appName)
	...
	if isKind {
		...
		lb, err := waitForIngressLB(ctx, kclient, timeout, cfg.Namespace, appName)
		if err != nil {
			t.Fatalf("failed to get ingress %s/%s load-balancer: %v", cfg.Namespace, appName, err)
		}
		t.Logf("ingress %s/%s has load-balancer %s", cfg.Namespace, appName, lb)
		testURL = fmt.Sprintf("http://%s/", lb)

		// Curl the ingress from a client pod.
		cliName := "test-client"
		if err := podWaitForHTTPResponse(ctx, kclient, cfg.Namespace, cliName, testURL, timeout); err != nil {
			t.Fatalf("failed to receive http response for %q: %v", testURL, err)
		}
		t.Logf("received http response for %q", testURL)
	}
	...
	if err := deleteContour(ctx, kclient, timeout, cntr.Name, cntr.Namespace); err != nil {
		t.Fatalf("failed to delete contour %s/%s: %v", cntr.Namespace, cntr.Name, err)
	}
	t.Logf("deleted contour %s/%s", cntr.Namespace, cntr.Name)

	// Ensure the envoy service is cleaned up automatically.
	if err := waitForServiceDeletion(ctx, kclient, timeout, cfg.Namespace, "envoy"); err != nil {
		t.Fatalf("failed to delete envoy service %s/envoy: %v", cfg.Namespace, err)
	}
	t.Logf("cleaned up envoy service %s/envoy", cfg.Namespace)

	// Delete the Contour namespace.
	if err := deleteNamespace(ctx, kclient, timeout, cfg.Namespace); err != nil {
		t.Fatalf("failed to delete namespace %s: %v", cfg.Namespace, err)
	}
	t.Logf("observed the deletion of namespace %s", cfg.Namespace)
}
...
func TestContourNodePortService(t *testing.T) {
	...
	if err := kclient.Get(ctx, key, svc); err != nil {
		t.Fatalf("failed to get service %s/%s: %v", cfg.Namespace, appName, err)
	}
	svc.Spec.Ports[0].Port = int32(81)
	if err := kclient.Update(ctx, svc); err != nil {
		t.Fatalf("failed to update service %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("updated service %s/%s", cfg.Namespace, appName)
	...
	if err := kclient.Get(ctx, key, ing); err != nil {
		t.Fatalf("failed to get ingress %s/%s: %v", cfg.Namespace, appName, err)
	}
	ing.Spec.DefaultBackend.Service.Port.Number = int32(81)
	if err := kclient.Update(ctx, ing); err != nil {
		t.Fatalf("failed to update ingress %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("updated ingress %s/%s", cfg.Namespace, appName)
...
func TestContourClusterIPService(t *testing.T) {
	...
	svcName := "envoy"
	ip, err := envoyClusterIP(ctx, kclient, cfg.Namespace, svcName)
	if err != nil {
		t.Fatalf("failed to get clusterIP for service %s/%s: %v", cfg.Namespace, svcName, err)
	}
	...
	// Ensure the default contour can be deleted.
	if err := deleteContour(ctx, kclient, timeout, cntr.Name, cntr.Namespace); err != nil {
		t.Fatalf("failed to delete contour %s/%s: %v", cntr.Namespace, cntr.Name, err)
	}
	t.Logf("deleted contour %s/%s", cntr.Namespace, cntr.Name)
...
func TestContourSpec(t *testing.T) {
	...
	if err := waitForContourStatusConditions(ctx, kclient, timeout, cfg.Name, cfg.Namespace, expectedContourConditions...); err != nil {
		t.Fatalf("failed to observe expected status conditions for contour %s/%s: %v", cfg.Namespace, testName, err)
	}
	t.Logf("observed expected status conditions for contour %s/%s", cfg.Namespace, testName)
...
func TestMultipleContours(t *testing.T) {
	testNames := []string{"test-multi-contour", "test-multi-contour-2"}
	...
	for _, testName := range testNames {
		...
		if err := deleteContour(ctx, kclient, timeout, testName, ns); err != nil {
			t.Fatalf("failed to delete contour %s/%s: %v", ns, testName, err)
		}
		t.Logf("deleted contour %s/%s", ns, testName)

		// Delete the Contour namespace.
		if err := deleteNamespace(ctx, kclient, timeout, ns); err != nil {
...
func TestGateway(t *testing.T) {
	...
	if err := newOperatorGatewayClass(ctx, kclient, gcName, cntr.Namespace, cntr.Name); err != nil {
	...
	gwName := "contour"
	appName := fmt.Sprintf("%s-%s", testAppName, testName)
	if err := newGateway(ctx, kclient, cfg.Namespace, gwName, gcName, "app", appName); err != nil {
		t.Fatalf("failed to create gateway %s/%s: %v", cfg.Namespace, gwName, err)
	}
	t.Logf("created gateway %s/%s", cfg.Namespace, gwName)

	// The gateway should report admitted.
	if err := waitForGatewayStatusConditions(ctx, kclient, timeout, gwName, cfg.Namespace, expectedGatewayConditions...); err != nil {
		t.Fatalf("failed to observe expected status conditions for gateway %s/%s: %v", cfg.Namespace, gwName, err)
	}
	...
	if err := newHTTPRouteToSvc(ctx, kclient, appName, cfg.Namespace, appName, "app", appName, "local.projectcontour.io", int32(80)); err != nil {
		t.Fatalf("failed to create httproute %s/%s: %v", cfg.Namespace, appName, err)
	}
	t.Logf("created httproute %s/%s", cfg.Namespace, appName)
	...
	if err := deleteGateway(ctx, kclient, timeout, gwName, cfg.Namespace); err != nil {
		t.Fatalf("failed to delete gateway %s/%s: %v", cfg.Namespace, gwName, err)
	}
...
func TestGatewayOwnership(t *testing.T) {
	...
	appName := fmt.Sprintf("%s-%s", testAppName, testName)
	if err := newGateway(ctx, kclient, cfg.Namespace, nonOwnedGateway, nonOwnedClass, "app", appName); err != nil {
		t.Fatalf("failed to create gateway %s/%s: %v", cfg.Namespace, nonOwnedGateway, err)
	}
	t.Logf("created gateway %s/%s", cfg.Namespace, nonOwnedGateway)

	// The gateway should not report scheduled.
	if err := waitForGatewayStatusConditions(ctx, kclient, timeout, nonOwnedGateway, cfg.Namespace, expectedNonOwnedGatewayConditions...); err != nil {
		t.Fatalf("failed to observe expected status conditions for gateway %s/%s: %v", cfg.Namespace, nonOwnedGateway, err)
	}
	...
	for _, gw := range gateways {
		// Ensure the gateway can be deleted and clean-up.
		if err := deleteGateway(ctx, kclient, timeout, gw, cfg.Namespace); err != nil {
			t.Fatalf("failed to delete gateway %s/%s: %v", cfg.Namespace, gw, err)
		}
	}
...
// upgrading the operator from the previous release to the current version/branch.
func TestOperatorUpgrade(t *testing.T) {
	...
	latest := fmt.Sprintf("docker.io/projectcontour/contour-operator:%s", latestRelease)
	if current == latest {
	...
	// Install the latest version of the Contour CRD.
	cmdPath, err := exec.LookPath("kubectl")
	if err != nil {
		t.Fatalf("failed to find kubectl path: %v", err)
	}
	url := fmt.Sprintf("github.com/projectcontour/contour-operator/config/crd?ref=%s", latestRelease)
	cmd := exec.Command(cmdPath, "apply", "-k", url)
	if err := cmd.Run(); err != nil {
		t.Fatalf("failed to downgrade crds: %v", err)
	}

	// Set the operator image to the latest release.
	...
	// Wait for the operator container to use the latest release image.
	...
	// Create the namespace used by Contour
	if err := newNs(ctx, kclient, cfg.Namespace); err != nil {
		t.Fatalf("failed to create namespace %s: %v", cfg.Namespace, err)
	}
	...
	// Simulate an upgrade from the latest release to the current branch.
	cmd = exec.Command(cmdPath, "apply", "-k", "../../config/crd")
	if err := cmd.Run(); err != nil {
		t.Fatalf("failed to upgrade crds: %v", err)
	}
	...
	// Wait for the contour containers to use the updated image.
	wantContourImage := config.DefaultContourImage
	if err := waitForImage(ctx, kclient, timeout, cfg.Namespace, "app", "contour", "contour", wantContourImage); err != nil {
		t.Fatalf("failed to observe image %s for deployment %s/contour: %v", wantContourImage, cfg.Namespace, err)
	}
	t.Logf("observed image %s for deployment %s/contour", wantContourImage, cfg.Namespace)
